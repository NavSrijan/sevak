...
class LLM:
    ...
    async def _load_history(self, session_id: str) -> tuple[list, str | None]:
    ...
    async def _save_history(self, session_id: str, messages: list, system_prompt: str | None = None) -> None:
    ...
    async def invoke(self, input: str, session_id: str, system_prompt: str | None = None) -> LLMResponse:
        message_history, stored_system_prompt = await self._load_history(session_id)
        ...
        await self._save_history(session_id, result.all_messages(), system_prompt=active_system_prompt)
